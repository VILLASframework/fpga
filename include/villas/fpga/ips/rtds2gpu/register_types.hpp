...
struct Rtds2GpuMemoryBuffer {
	...
	static constexpr
	size_t rawValueCount = N * (sizeof(T) / 4);
	...
	//static constexpr
	//size_t doorbellOffset = offsetof(Rtds2GpuMemoryBuffer, doorbell);
	//static constexpr
	//size_t dataOffset = offsetof(Rtds2GpuMemoryBuffer, data);

	// HACK: This might break horribly, let's just hope C++17 will be there soon
	static constexpr
	size_t dataOffset = 0;
	static constexpr
	size_t doorbellOffset = N * sizeof(Rtds2GpuMemoryBuffer::data);
