...
static FpgaDeviceFactory villas::fpga::FpgaDeviceFactory;

static const kernel::pci::Device defaultFilter((kernel::pci::Id(FPGA_PCI_VID_XILINX, FPGA_PCI_PID_VFPGA)));

FpgaDevice::List
FpgaDeviceFactory::make(json_t *json, std::shared_ptr<kernel::pci::DeviceList> pci, std::shared_ptr<kernel::vfio::Container> vc)
{
	...
	json_object_foreach(json, card_name, json_card) {
		...
		auto card = std::unique_ptr<FpgaDevice>(create());
...
ip::Core::Ptr
FpgaDevice::lookupIp(const ip::IpIdentifier &id) const
{
	for (auto &ip : ips) {
		if (*ip == id) {
			return ip;
...
bool
FpgaDevice::mapMemoryBlock(const MemoryBlock &block)
{
